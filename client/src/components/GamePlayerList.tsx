...
export default function GamePlayerList({ avalon, onSelectedPlayers }: GamePlayerListProps) {
  ...
  const enableCheckboxes = (name: string): boolean => {
    return (avalon.game.phase === 'TEAM_PROPOSAL' && avalon.game.currentProposer === avalon.user.name) ||
  ...
  const selectedForMission = (name: string): boolean => {
    return (avalon.game.phase === 'PROPOSAL_VOTE' || avalon.game.phase === 'MISSION_VOTE') &&
      avalon.game.currentProposal.team.includes(name);
  };

  const hasVoted = (name: string): boolean => {
    return (avalon.game.phase === "PROPOSAL_VOTE") &&
        ...
        <div key={playerName} className={styles.playerItem}>
          ...
          <div className={styles.statusColumn}>
            {avalon.game.currentProposer === playerName && (
              <div className={styles.crownContainer} title={`${playerName} is proposing the next team`}>
                <span className={styles.crown} style={{ color: crownColor }}>👑</span>
                <span className={styles.proposalNumber}>{avalon.game.currentProposalIdx + 1}</span>
              </div>
            )}
          ...
          <div className={styles.iconColumn}>
            {tooltipText(playerName) && (
              <div className={styles.statusIcons} title={tooltipText(playerName) || ''}>
                ...
                {approvedProposal(playerName) && !hasVoted(playerName) && !waitingOnVote(playerName) && (
                  <span className={styles.approvedIcon}>👍</span>
                )}
                {rejectedProposal(playerName) && !hasVoted(playerName) && !waitingOnVote(playerName) && (
