...
const ActionPane: React.FC<ActionPaneProps> = ({ avalon, selectedPlayers }) => {
  ...
  return (
    <div>
      {teamProposal && <TeamProposalAction avalon={avalon} playerList={selectedPlayers} />}
      {teamVote && <TeamVoteAction avalon={avalon} />}
      {missionAction && <MissionAction avalon={avalon} />}
      {assassinationPhase && <AssassinationAction playerList={selectedPlayers} avalon={avalon} />}
    </div>
  );
};
